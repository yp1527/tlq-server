...
public class ServerResponseThread implements Runnable {
    ...
    public ServerResponseThread(Socket socket, SocketServerResponseInterface socketServerResponseInterface) {
        ...
        System.out.println("用户：" + userIP+ " 连接了服务端,当前在线客户端:" + onLineClient.size());
    }
    ...
    public void stop() {
        try {
            ...
            System.out.println("用户：" + userIP+ " 退出,当前在线客户端:" + onLineClient.size());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
            ...
            try {
                ...
                while (!isCancel) {
                   ...
                   InputStream inputStream=socket.getInputStream();
                    byte[] bytes = new byte[2048]; // 一次读取一个byte
                    String msg = "";
                    ...
                    if (msg != null) {
                        if (msg.contains("ping")) {
                            System.out.println("服务端收到客户端心跳包...");
                            lastReceiveTime = System.currentTimeMillis();
                            // socketServerResponseInterface.clientOnline(userIP);
                        } else {
                            // msg = "用户" + userIP + " : " + msg;
                            System.out.println("服务端收到客户端消息:"+msg);
                            if(msg.equals("subTopic")){
                                //模拟客户端订阅消息
                                //objectOutputStream.writeObject("resultkjljljjlj");
                                addMessage("{\"action\":\"test_temp\",\"data\":{\"deviceItems\":[{\"deviceCode\":\"20033001\",\"fridgeTemp\":35}]}}");
                               // addMessage("{\"action\":\"test_temp\",\"data\":{\"deviceItems\":[{\"deviceCode\":\"20033002\",\"fridgeTemp\":36}]}}");
                            }
                            // addMessage(msg);
                            // socketServerResponseInterface.clientOnline(userIP);
